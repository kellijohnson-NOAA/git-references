...
\(n\) = allometric scaling of energy costs, where von Bertalanffy ((<span class="citeproc-not-found" data-reference-id="vonbertalanffy_1938">**???**</span>)) assumed \(n = 1\) (linear); standard metabolism rates suggest that \(n\) should be nonlinear and closer to 0.8, but if so then there is no closed form solution to the integral of the rate equation and standard metabolism does not account for other energy costs (e.g., swimming, reproduction).
